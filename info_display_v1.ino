...
void printMessage(String entity_message) {
...
void displayElectricity() {
  ...
  if (el_msg.doTask()) { 
    current_message = getEntityStatus("sensor.sensor_messagedisplay1");
  } else {
  ...
  if (currentHour!=timeinfo.tm_hour || displaymode_change || (current_message != last_message )) {
  ...
  if (el_arc.doTask() || current_message != last_message) { 
    printConsumption();
    printMessage(current_message);
  }
